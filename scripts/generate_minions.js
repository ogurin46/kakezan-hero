'use strict';
// 使い方: node scripts/generate_minions.js
// ウェーブ戦用のザコモンスター6体を生成する（各ステージにランダムで2体登場）
...
const CHARS = [
  { file: 'minion_1.png', // プルプルン
    prompt: 'cute chibi purple ghost monster, round floating body with wavy bottom, big friendly eyes, tiny stubby arms, mischievous smile' + STYLE },
  { file: 'minion_2.png', // サボテンダ
    prompt: 'cute chibi green cactus monster, round body, small soft spikes, pink flower on head, happy smiling face, stubby arms' + STYLE },
  { file: 'minion_3.png', // ビリビリン
    prompt: 'cute chibi yellow lightning imp monster, spiky electric hair, round body, small sparks on hands, energetic grin' + STYLE },
  { file: 'minion_4.png', // シズクン
    prompt: 'cute chibi blue water drop monster, glossy teardrop shaped body, big round sparkling eyes, tiny splashing hands' + STYLE },
  { file: 'minion_5.png', // ニャゴラス
    prompt: 'cute chibi pink cat monster, big pointed ears, fluffy tail, playful standing pose, cheeky fanged smile' + STYLE },
  { file: 'minion_6.png', // コンタロー
    prompt: 'cute chibi orange fox spirit monster, fluffy tail with white tip, pointed ears, clever confident smile, standing pose' + STYLE },
];
...
async function main() {
  ...
  for (let i = 0; i < CHARS.length; i++) {
    process.stdout.write(`[${i + 1}/${CHARS.length}] ${CHARS[i].file.padEnd(14)} 生成中... `);
    try {
